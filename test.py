...
import io
from credentials import connect_str_acces, container_name_acces


# Initialize Blob Service Client and Container Client
blob_service_client = BlobServiceClient.from_connection_string(connect_str_acces)
...
def main():
    ...
    if is_registered:
        ...
        st.warning("IP Address not registered.")
        pin = st.text_input("Enter the PIN to register your IP:", type='password')
        if pin == pin_code:  # Replace '1234' with the desired PIN code
            register_ip(user_ip)
...
if main() == 1:
    st.title("Continue myntydanalyser")
else:
    main()
